...
comercio_mundial <- read_csv("BACI/BACI_HS92_Y2023_V202501.csv")
...
# Agrupamos según exportador
comercio_mundial <- comercio_mundial %>%
  group_by(i, k) %>%
  summarise(v = sum(v))

# Levantamos el diccionario de países
paises_baci <- read_csv("BACI//country_codes_V202501.csv")
...
# Creamos nueva columna con el total de exportaciones del mundo
comercio_mundial <- comercio_mundial %>%
